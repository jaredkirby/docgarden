...
LOCAL_ARTIFACT_NAMES = {"AGENTS.md", "README.md", "pyproject.toml", "uv.lock"}
    ...
    for source in sources:
        ...
        findings.append(
            Finding.open_issue(
                ...
                suffix=f"source-{abs(hash(source))}",
            )
        )
    return findings
    ...
    for command in extract_validation_commands(document.body):
        ...
        findings.append(
            Finding.open_issue(
                ...
                suffix=f"command-{abs(hash(command))}",
            )
        )
    return findings
...
def resolve_repo_artifact(repo_root: Path, value: str) -> Path | None:
    ...
    path = Path(candidate)
    if path.name in LOCAL_ARTIFACT_NAMES or path.suffix in LOCAL_ARTIFACT_SUFFIXES:
...
def extract_sections(body: str) -> list[tuple[str, str]]:
    ...
    for index, match in enumerate(matches):
        ...
        sections.append(
            (normalize_heading(match.group(2)), body[start:end].strip())
        )
    return sections
